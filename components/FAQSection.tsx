...
const FAQSection: React.FC<FAQSectionProps> = ({ language }) => {
  ...
  return (
    <section 
      id="faq-section"
      className="bg-white dark:bg-slate-900 rounded-2xl shadow-lg border border-slate-200/90 dark:border-slate-800 overflow-hidden my-8 print:hidden transition-colors"
    >
      {/* Header Banner */}
            ...
            return (
              <div
                ...
                className={`rounded-xl border transition-all duration-200 overflow-hidden ${
                  isOpen
                    ? 'bg-blue-50/40 dark:bg-blue-950/30 border-blue-300 dark:border-blue-800 shadow-sm ring-1 ring-blue-400/20'
                    : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600 hover:bg-slate-50/50 dark:hover:bg-slate-700'
                }`}
              >
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${
                      isOpen ? 'bg-blue-600 text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300'
                    }`}>
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-0.5">
                        <span className={`text-[10px] font-bold px-2 py-0.2 rounded-full ${
                          isOpen ? 'bg-blue-200/80 dark:bg-blue-900 text-blue-900 dark:text-blue-200' : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300'
                        }`}>
                          {isArabic ? faq.badge?.ar : faq.badge?.en}
                        </span>
                      </div>
                      <h3 className="text-xs md:text-sm font-bold text-slate-900 dark:text-white leading-snug">
                  ...
                  <div className="flex-shrink-0 text-slate-400 p-1">
                    {isOpen ? <ChevronUp className="w-4 h-4 text-blue-600 dark:text-blue-400" /> : <ChevronDown className="w-4 h-4" />}
                  </div>
                </button>

                {/* Answer Content */}
                {isOpen && (
                  <div className={`px-4 pb-4 pt-1 border-t border-slate-200/60 dark:border-slate-700 ${isArabic ? 'text-right' : 'text-left'}`}>
                    <p className="text-xs md:text-sm text-slate-700 dark:text-slate-300 leading-relaxed pt-2">
                      {isArabic ? faq.answer.ar : faq.answer.en}
                    </p>

                    {faq.highlight && (
                      <div className="mt-3 p-2.5 rounded-lg bg-white dark:bg-slate-900 border border-blue-200/80 dark:border-blue-800/80 text-xs font-semibold text-blue-900 dark:text-blue-200 flex items-start gap-2 shadow-2xs">
      ...
      <div className="p-4 md:p-5 bg-slate-50 dark:bg-slate-800/80 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
