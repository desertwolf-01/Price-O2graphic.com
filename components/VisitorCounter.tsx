...
const VisitorCounter: React.FC<VisitorCounterProps> = ({ t, language }) => {
  ...
  return (
    <div className={`flex items-center gap-2 text-xs font-medium text-slate-400 dark:text-slate-500 border-l border-slate-200 dark:border-slate-800 pl-4 h-4 ${language === 'ar' ? 'flex-row-reverse border-l-0 pl-0 border-r pr-4' : ''}`}>
      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
      </svg>
      <span>
        {t.visitorCount}: <span className="text-slate-600 dark:text-slate-300 font-bold">{formatNumber(count)}</span>
      </span>
    </div>
  );
};
